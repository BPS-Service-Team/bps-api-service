...
export const useFetchOrders = (queries = [], skip = 0, limit = 10) => {
  ...
  useEffect(() => {
    ...
    const aQueries = aTemp.filter(
      oElem => oElem.value || typeof oElem.value === 'number'
    );

    getOrders(aQueries, params.skip, params.limit).then(response => {
      if (response.ok) {
