...
import { getAgfSingle } from '../Services/API';

export const useFetchAgfSingle = (queries = [], skip = 0, limit = 10) => {
  ...
  useEffect(() => {
    setLoading(true);
    getAgfSingle(params.queries, params.skip, params.limit).then(response => {
      if (response.ok) {
  ...
  async function update() {
    setLoading(true);
    let response = await getAgfSingle(
      params.queries,
      params.skip,
      params.limit
    );
